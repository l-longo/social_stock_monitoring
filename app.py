...
if avg_return is not None:
    ...
    st.subheader(f"Variance (relative to last year variance) after alert for {ticker.upper()}: {var_return:.5}/{np.var(df_financial['Return']):.5}. Variance during last year: {np.var(df_financial['Return']):.5}")
