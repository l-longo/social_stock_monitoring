...
st.markdown("**Latest public update: April 30 2025**.")
...
# Select ticker
ticker = st.selectbox("Select a ticker:", ['tsla', 'gme', 'amc', 'nvda', 'amzn', 'googl', 'meta', 'nflx', 'cost', 'intc', 'sbux'])
...
# Load financial data
df_financial = pd.read_excel(f'{path_saved}update_financial_{ticker}_360_20.xlsx')
df = pd.read_excel(f'{path_saved}update_df_{ticker}_{I}_{network_days}.xlsx', index_col=0)

# Load alert dates
file_path_alter_dates = f'{path_saved}update_alert_dates_{ticker}_{I}_{network_days}.txt'
...
if avg_return is not None:
    st.subheader(f"Average return (next 20 consecutive days) after alert for {ticker.upper()}: {avg_return:.2%}")
    st.subheader(f"Variance (relative to last year's variance) after alert for {ticker.upper()}: {var_return/np.var(df_financial['Return']):.5}.")
